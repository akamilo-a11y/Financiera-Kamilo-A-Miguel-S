"""
Módulo Analítico - Finanzas Personales
Procesa los datos financieros con Pandas y Scikit-learn para:
  1. Predecir el gasto del próximo mes (Regresión Lineal).
  2. Detectar anomalías estadísticas en los gastos (Z-Score).
"""
import pandas as pd
import numpy as np
...
def cargar_datos(conexion, id_usuario: int) -> pd.DataFrame:
    """Carga los movimientos del usuario desde la BD en un DataFrame."""
    query = """
        SELECT id_movimiento, fecha, tipo, monto, id_categoria
...
def predecir_gasto_proximo_mes(df: pd.DataFrame) -> dict:
    """Estima el gasto acumulado del siguiente mes mediante regresión lineal."""
    ...
    # Bordes: pocos datos -> promedio simple
    if cant_meses < 2:
...
def detectar_anomalias(df: pd.DataFrame, umbral_z: float = 1.5) -> list:
    """Marca movimientos de gasto cuya desviación (|Z|) supera el umbral."""
    ...
    for _, row in anomalias.iterrows():
        resultado.append({
            "id_movimiento": int(row.get('id_movimiento', 0)),
